				...
				{#each watchlist as item (item.movie.id)}
					<div class="flex items-center justify-between gap-3 p-3 rounded-lg bg-purple-900/30">
						<div class="flex-1 min-w-0">
							<p class="font-medium truncate">{item.movie.title} ({item.movie.year})</p>
							<p class="text-sm text-purple-300 truncate">{item.explanation}</p>
						</div>
						<div class="shrink-0">
							<Button onclick={() => onRemove(item.movie.id)} size="sm" variant="destructive">
								<Trash class="w-4 h-4" />
							</Button>
						</div>
					</div>
