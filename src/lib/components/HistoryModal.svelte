<script lang="ts">
	...
	import { searchHistory, loadHistory, deleteHistory } from '$lib/stores/discover';
	...
	const t: Record<Language, HistoryTranslations> = {
		el: {
			...
			description: 'Εδώ είναι οι πρόσφατες αναζητήσεις σας. Μπορείτε να φορτώσετε ή να διαγράψετε.',
			load: 'Φόρτωση',
		...
		en: {
			...
			description: 'Here are your recent searches. You can load a previous result or delete it.',
			load: 'Load',
...
<Dialog.Root bind:open>
	<Dialog.Content class="bg-slate-900/80 border-purple-500/30 text-white">
		<Dialog.Header>
		...
		<div class="max-h-[60vh] overflow-y-auto space-y-3 pr-4">
			...
			{:else}
				{#each $searchHistory as item (item.id)}
					<div class="flex items-center justify-between gap-3 p-3 rounded-lg bg-purple-900/30">
						<div class="flex-1 min-w-0">
							<p class="font-medium truncate">{item.title}</p>
							<p class="text-sm text-purple-300 truncate">{item.summary}</p>
							<p class="text-xs text-purple-400/70">
								{new Date(item.timestamp).toLocaleString(settings.lang)}
							</p>
						</div>
						<div class="flex gap-2 shrink-0">
							<Button onclick={() => handleLoad(item)} size="sm" variant="outline">
